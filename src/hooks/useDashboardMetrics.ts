  ...
  useEffect(() => {
    const fetchMetrics = async () => {
      ...
      try {
        ...
        const totalInvestimento = projetos?.reduce((sum, projeto) =>
          sum + (Number(projeto.valor_aporte_projeto) || 0), 0
        ) || 0;

        const valorTotalProjetos = projetos?.reduce((sum, projeto) =>
          sum + (Number(projeto.valor_total_projeto) || 0), 0
        ) || 0;

        const pessoasImpactadas = projetos?.reduce((sum, projeto) =>
          sum + (projeto.numero_pessoas_impactadas || 0), 0
        ) || 0;

        // Contar localidades únicas
        const localizacoes = new Set(
          projetos?.map(p => p.localidade_projeto).filter(Boolean)
        );
